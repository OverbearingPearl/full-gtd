;;; full-gtd-todo-keywords-test.el --- Tests for custom org-todo-keywords  -*- lexical-binding: t; -*-
...
(ert-deftest full-gtd-todo-keywords-test-default-todo-keyword ()
  ...
  (let ((org-not-done-keywords '("NEXT"))
    ...
    (should (string= (full-gtd-core--default-todo-keyword) "NEXT")))
  (let ((org-not-done-keywords nil))
    (should (string= (full-gtd-core--default-todo-keyword) "TODO"))))
  ...
  (let* ((org-todo-keywords '((sequence "TODO" "DOING" "DONE")))
    ...
    (unwind-protect
        (progn
          ...
          (let ((contexts (full-gtd-core-collect-contexts
                           (expand-file-name "action.org" full-gtd-init-base-directory))))
    ...
    (unwind-protect
        (progn
          (write-region "* TODO Task :office:\n:PROPERTIES:\n:ID: t1\n:END:\n* DOING Another :DOING:\n:PROPERTIES:\n:ID: t2\n:END:\n"
                        nil
                        (expand-file-name "action.org" full-gtd-init-base-directory))
          (let ((candidates (full-gtd-domain--collect-context-candidates)))
            (should (member "@office" candidates))
            (should-not (member "@DOING" candidates))))
      (delete-directory full-gtd-init-base-directory t))))

(ert-deftest full-gtd-todo-keywords-test-multi-sequence-lowercase ()
...
(provide 'full-gtd-todo-keywords-test)

;;; full-gtd-todo-keywords-test.el ends here
